...
public final class Payment implements Comparable<Payment> {
	...
	public Payment(Date date, Currency currency, double amount) {
		...
		if (date == null || currency == null) {
			throw new IllegalArgumentException(
					"date and currency must not be null");
		}
	}
	...
	public Payment add(Payment other) {
		...
		throw new IllegalArgumentException(
				"Cannot add payments on different date or currency");
	}

	@Override
	public int hashCode() {
		return currency.hashCode() + date.hashCode()
				+ (int) (Double.doubleToLongBits(amount));
	}
